...
3b. **标题把 `[页码]` 放在末尾（vol_4 的写法）**：vol_4 的远端标题绝大多数是
   `ရုပ် (၂၈) ပါး = ရူပက္ခန္ဓကထာ အခန်း[010]` 这种 key 在**行尾**的形式，
   开头式一个都匹配不上（374 行里 181 行 `parse_fail`）。所以在开头式失败后
   回退到行尾式：
   ```
   TITLE_KEY_TAIL_RE = r"(\[[^\]]+\][A-Za-z]?)[\s​‌‍]*$"
   ```
   缅文标题末尾常跟零宽字符（U+200B/C/D），必须一并吃掉，否则 `[217]‌‌` 这种
   匹配不上。**只在开头式解析失败时回退**，vol_4 匹配数因此从 182 → 342。

   ⚠ **带 `NGP-Vol` 前缀的标题不做尾部回退**（`NGP_PREFIX_RE`）。开头式解析不出
   key 的 NGP 前缀标题，就是下面「位置后缀」那类后台遗留节点：`NGP-Vol-5[332]`
   一旦被尾部回退解析成 `[332]`，就会与真正的 `[332] ကြောက်မက်ဖွယ်…`（已上传
   18 句）撞成 `dup_remote`，把一篇本来 `done` 的文章挡下来。实测踩过一次。
